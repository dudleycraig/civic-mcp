...
(defn view
  [{configuration :configuration match-name :match-name routes :routes session :session}]
  (let [{user-email :user/email} @(:state session)]
       ...
       (if session
         [:div.flex.items-center.gap-x-2
          ...
          [:button.btn.btn-outline.btn-sm
           {:on-click (partial logout-handler configuration session)
