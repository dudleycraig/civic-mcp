  ...
  (let [match-name (-> match :data :name)
    ...
    [:<>
     [ui.views.components.main-header/view {:user-email user-email :on-logout on-logout :match-name match-name :routes routes}
      [ui.views.components.main-navigation/view {:key "main-menu" :routes routes :match-name match-name}]]
     [:main.flex-1.relative {:role "main"}
    ...
    (let [{{data-theme :data-theme} :ui} configuration
      ...
      (if match
        [ui.views.components.shell/view {:data-theme data-theme}
         (case match-layout
           :private [private-view {:match match :routes routes :session session} [match-view {:key (name match-name) :match match}]]
           :public  [public-view  {}                                             [match-view {:key (name match-name) :match match}]]
           [transitional-view])]
        [transitional-view]))))
